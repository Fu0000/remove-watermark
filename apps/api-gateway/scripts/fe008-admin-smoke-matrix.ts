...
const FE008_SMOKE_SCRIPT = resolve(CURRENT_DIR, "fe008-admin-smoke.ts");
  ...
  return raw
    ...
    .map((entry) => {
      ...
      if (index <= 0 || index >= entry.length - 1) {
        throw new Error(`FE008_MATRIX_TARGETS 格式错误：${entry}，应为 name=url`);
      }
...
function resolveTargets(): SmokeTarget[] {
  const customTargets = process.env.FE008_MATRIX_TARGETS?.trim() || process.env.SMOKE_MATRIX_TARGETS?.trim();
  if (customTargets) {
...
async function preflight(baseUrl: string): Promise<string> {
  ...
  try {
    const response = await fetch(`${baseUrl}/v1/system/capabilities`, {
      method: "GET",
      headers: {
        Authorization: "Bearer fe008_matrix",
        "X-Request-Id": `fe008_matrix_preflight_${Date.now()}`
      },
...
function runFe008Smoke(target: SmokeTarget): Promise<SmokeResult> {
  const startedAt = Date.now();
  return new Promise(async (resolveResult) => {
    const preflightState = await preflight(target.baseUrl);
    const nodeArgs = ["--import", "tsx", FE008_SMOKE_SCRIPT];
    ...
    child.on("error", (error) => {
      outputChunks.push(`[fe008-matrix] spawn error: ${error.message}`);
    });
...
function buildReport(targets: SmokeTarget[], results: SmokeResult[]) {
  ...
  const lines: string[] = [];
  lines.push("# fe008 admin smoke matrix report");
  lines.push("");
...
async function main() {
  ...
  if (!targets.length) {
    throw new Error("未解析到任何目标环境，请检查 FE008_MATRIX_TARGETS 或相关环境变量");
  }

  console.log(`[fe008-admin-smoke-matrix] targets=${targets.map((item) => `${item.name}:${item.baseUrl}`).join(", ")}`);

  const results: SmokeResult[] = [];
  for (const target of targets) {
    console.log(`[fe008-admin-smoke-matrix] running target=${target.name} baseUrl=${target.baseUrl}`);
    const result = await runFe008Smoke(target);
    results.push(result);
    console.log(
      `[fe008-admin-smoke-matrix] target=${result.name} status=${result.status} preflight=${result.preflight} durationMs=${result.durationMs}`
    );
  }

  const report = buildReport(targets, results);
  const writeReport = process.env.FE008_MATRIX_WRITE_REPORT !== "0";
  let reportPath = "";

  if (writeReport) {
    mkdirSync(REPORT_DIR, { recursive: true });
    reportPath = resolve(REPORT_DIR, `fe008-admin-smoke-matrix-${new Date().toISOString().replace(/[:.]/g, "-")}.md`);
    writeFileSync(reportPath, report, "utf8");
    console.log(`[fe008-admin-smoke-matrix] report=${reportPath}`);
  }

  const failed = results.filter((item) => item.status === "failed");
  if (failed.length) {
    console.error(`[fe008-admin-smoke-matrix] failed targets=${failed.map((item) => item.name).join(", ")}`);
    process.exitCode = 1;
    return;
  }

  console.log("[fe008-admin-smoke-matrix] all targets passed");
  if (!writeReport) {
    console.log(report);
  }
}
...
main().catch((error) => {
  if (error instanceof Error) {
    console.error("[fe008-admin-smoke-matrix] failed:", error.message);
  } else {
    console.error("[fe008-admin-smoke-matrix] failed:", String(error));
  }
  process.exitCode = 1;
});
