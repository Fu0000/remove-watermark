...
6. 若提前落地 `V1.1` 能力，必须通过 Feature Flag 灰度并同步 `riskFlags` 披露。

## 4. 技术路线约束

1. 控制面：Node.js 20 + NestJS + Fastify。
2. 推理面：Triton 推理服务（Node 调用）；灰度阶段可引入 Python inference-gateway 作为过渡层。
3. 数据库：PostgreSQL。
4. 队列：BullMQ + Redis。
5. 对象存储：MinIO（S3 兼容）。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| v1.2 | 2026-02-22 | 增加“V1.1 能力灰度前置”约束与 inference-gateway 过渡策略 |
