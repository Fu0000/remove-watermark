...
| INT-006 | 订阅/配额扣减联调 | 前后端+测试+支付 | 2026-03-24 | 2026-04-07 | In Review | 扣减一致率 100% | 本地 Prisma 已覆盖 payment-callback(PAID/REFUNDED) + 退款幂等 + 配额回滚；已新增 `test:shared-smoke:local-stack` 一键复验脚本 |
...
| Prisma 模式 shared 一键双进程验收（本轮，本地） | `DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:5432/remove_watermark REDIS_URL=redis://127.0.0.1:6379 SHARED_BASE_URL=http://127.0.0.1:3000 pnpm --filter @apps/api-gateway test:shared-smoke:local-stack` | `passed` | 已固化为单命令：`reset-user -> start api -> start worker -> run shared-smoke -> report` |
...
## 48. 本次执行回填（BE-009 第一阶段：删除与审计最小闭环）

- 任务编号：`DEV-20260222-BE009-COMPLIANCE-MINIMAL`
...
- 截止时间：`2026-04-12`
- 当前状态：`In Progress`
- 阻塞项：shared/staging 云端 PostgreSQL 与联调地址待接入（云端验证后置）
- 风险等级：中
- 改动范围：
  ...
  - `/Users/codelei/Documents/ai-project/remove-watermark/apps/api-gateway/src/modules/app.module.ts`
  - `/Users/codelei/Documents/ai-project/remove-watermark/apps/api-gateway/prisma/schema.prisma`
  - `/Users/codelei/Documents/ai-project/remove-watermark/apps/api-gateway/prisma/migrations/20260222100000_add_compliance_tables/migration.sql`
  - `/Users/codelei/Documents/ai-project/remove-watermark/apps/api-gateway/test/contract.spec.ts`
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/rd-progress-management.md`
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/change-log-standard.md`
- 实施摘要：
  - 落地合规模块 `compliance`，补齐以下接口最小闭环并保持 Header 约束一致：
    - `DELETE /v1/assets/{assetId}`（软删除 + 异步清理占位）
    - `DELETE /v1/tasks/{taskId}`（删除任务展示记录）
    - `POST /v1/account/delete-request`（返回 `requestId/status/eta`）
  - 新增审计日志写入能力，覆盖上传策略创建、素材删除、任务展示删除、账户删除申请。
  - 新增合规幂等存储（`compliance_idempotency`），支持同键重放与载荷冲突拦截（`40901`）。
  - 增加 `task_view_deletions` 以实现“删除展示不影响任务审计/账务主数据”的隔离策略。
- 测试证据：
  - `pnpm --filter @apps/api-gateway prisma:generate`：通过
  - `pnpm --filter @apps/api-gateway typecheck`：通过
  - `pnpm --filter @apps/api-gateway test:unit`：通过（`2/2`）
  - `DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:5432/remove_watermark pnpm --filter @apps/api-gateway exec prisma migrate deploy --schema prisma/schema.prisma`：通过（应用 `20260222100000_add_compliance_tables`）
  - `DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:5432/remove_watermark TASKS_STORE=prisma SUBSCRIPTIONS_STORE=prisma WEBHOOKS_STORE=prisma COMPLIANCE_STORE=prisma pnpm --filter @apps/api-gateway test:contract`：通过（`21/21`）
  - 双进程（`api-gateway + worker-orchestrator`）下执行 `test:shared-smoke`：通过
- 联调结果：
  - 本地契约层已覆盖删除与审计主链路，具备 shared/staging 地址替换后的复跑能力。
- 遗留问题：
  - 云端环境下 `audit_logs` 保留策略（180 天）与清理任务尚未实装调度。
  - 账户删除申请后续执行态（`PROCESSING/DONE`）与异步作业衔接待下一阶段实现。
- 风险与回滚：
  - 风险：若云端数据库版本与本地不一致，`JSONB` 字段或索引行为可能存在差异。
  - 回滚：回退 `compliance` 模块与相关迁移，恢复到无删除入口版本；台账状态回退 `BE-009 -> Backlog`。
- 下一步：
...
## 50. 本次执行回填（FE-007 账户隐私页：删除申请 + 审计查询）

- 任务编号：`DEV-20260222-FE007-ACCOUNT-PRIVACY`
...
- 截止时间：`2026-04-06`
- 当前状态：`In Review`
- 阻塞项：无（云端 shared/staging 验证后置发布前）
- 风险等级：中
- 改动范围：
  - `/Users/codelei/Documents/ai-project/remove-watermark/apps/user-frontend/src/services/compliance.ts`
  - `/Users/codelei/Documents/ai-project/remove-watermark/apps/user-frontend/src/pages/account/index.tsx`
  - `/Users/codelei/Documents/ai-project/remove-watermark/apps/user-frontend/src/pages/account/index.scss`
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/rd-progress-management.md`
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/change-log-standard.md`
- 实施摘要：
  - 新增前端合规服务层：账户删除申请创建、删除申请列表/详情查询、审计日志查询。
  - 账户页改造为可联调版本：删除申请提交表单、状态筛选列表、详情面板、审计日志过滤与列表。
  - 页面样式补齐多端适配（`rpx + h5 px`），保持信息结构一致，仅做交互样式差异化适配。
  - 复用统一请求层与 `Idempotency-Key` 生成策略，保持 Header 约束一致。
- 测试证据：
  - `pnpm --filter @apps/user-frontend typecheck`：通过
  - `pnpm --filter @apps/user-frontend build:h5`：通过（`2 warnings`，为既有包体告警）
- 联调结果：
  - 本地地址（`http://127.0.0.1:3000`）下，`POST /v1/account/delete-request` 与 `GET /v1/account/delete-requests*`、`GET /v1/account/audit-logs` 已可在页面侧触发与展示。
- 遗留问题：
  - 暂未接入“素材删除/任务删除”页面级快捷入口（接口已在后端可用）。
  - 云端 shared/staging 环境的 FE-007 验证证据待发布前统一补齐。
- 风险与回滚：
  - 风险：账户页查询项较多，后续需评估分页与虚拟列表策略以控制长列表渲染开销。
  - 回滚：回退 `account` 页面与 `services/compliance.ts` 改动，恢复占位页版本。
- 下一步：
...
## 59. 本次执行回填（FE-008 Webhook 运维切换 `/admin/*`）

- 任务编号：`DEV-20260222-FE008-ADMIN-WEBHOOKS`
- 需求映射：`FR-012`、`NFR-006`
- 真源引用：
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/api-spec.md`
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/admin-framework.md`
...
- 实施摘要：
  ...
  - 管理端 RBAC 新增 `admin:webhook:read`、`admin:webhook:retry`，并按 `admin/operator/auditor` 分层授权。
  - 管理端 Webhook 页面服务层切换到 `/admin/webhooks/*`，统一走服务端代理与 `X-Admin-Secret` 注入链路。
  - retry 动作新增后台审计动作 `admin.webhook.retry`。
- 测试证据：
  - `pnpm --filter @apps/api-gateway typecheck`：通过
  - `pnpm --filter @apps/api-gateway test:contract`：通过（`27/27`，含 `/admin/webhooks/*` RBAC 用例）
  - `pnpm --filter @apps/admin-console typecheck`：通过
  - `pnpm --filter @apps/admin-console build`：通过
- 联调结果：
  - 本地地址口径下，FE-008 的 Webhook 运维链路已从 `v1` 用户侧接口切换到 `/admin/*` 管理端契约。
- 遗留问题：
  - `userId` 当前默认 `u_1001`（可通过 query 指定），后续需在统一身份体系中切换到真实租户/用户上下文。
- 风险与回滚：
  - 风险：若误配 `userId` 查询参数，可能出现管理端看不到目标投递记录。
  - 回滚：回退 `admin.controller` 与 `admin-console` webhooks 服务层路径，恢复到 `/v1/webhooks/*` 读写路径。
- 下一步：
  - 继续补 FE-008 smoke（本地地址）并准备 shared/staging 最终验收切换。

## 60. 本次执行回填（FE-008 Webhook 运维改为上下文驱动）

- 任务编号：`DEV-20260222-FE008-ADMIN-SCOPE-CONTEXT`
- 需求映射：`FR-012`、`NFR-006`
- 真源引用：
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/api-spec.md`
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/admin-framework.md`
...
- 改动范围：
  ...
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/change-log-standard.md`
- 实施摘要：
  - 后端 `/admin/webhooks/deliveries*` 去除默认 `u_1001`，改为显式上下文必填。
  - 支持上下文参数：
    - 推荐：`scopeType=user|tenant` + `scopeId`
    - 兼容：`userId`/`tenantId`（过渡期）
  - 管理端 Webhook 页面新增上下文选择（用户/租户）与上下文 ID 输入，查询与重试均强制透传。
  - 缺少上下文参数时返回 `40001`，阻断“默认用户误操作”。
- 测试证据：
  - `pnpm --filter @apps/api-gateway typecheck`：通过
  - `pnpm --filter @apps/api-gateway test:contract`：通过（`28/28`，含 scope 必填用例）
  - `pnpm --filter @apps/admin-console typecheck`：通过
  - `pnpm --filter @apps/admin-console build`：通过
- 联调结果：
  - 本地地址口径下，Webhook 运维已完成“管理端选择上下文 -> `/admin/webhooks/*` 查询/重试”的闭环。
- 遗留问题：
  - 当前 `tenant` 口径在租户模型未落地前仍走单维过滤语义，后续需在数据层补齐真实租户归属字段。
- 风险与回滚：
  - 风险：若运营未填写上下文，页面会直接拦截并提示，不再自动回落默认用户。
...
## 70. 本次执行回填（INT-006 本地一键双进程验收脚本）

- 任务编号：`DEV-20260222-INT006-LOCAL-STACK`
- 需求映射：`FR-008`、`NFR-006`
- 真源引用：
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/api-spec.md`
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/testing-workflow.md`
...
- 风险等级：低
- 改动范围：
  - `/Users/codelei/Documents/ai-project/remove-watermark/apps/api-gateway/scripts/shared-smoke-local-stack.ts`
  - `/Users/codelei/Documents/ai-project/remove-watermark/apps/api-gateway/package.json`
  - `/Users/codelei/Documents/ai-project/remove-watermark/AGENTS.md`
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/rd-progress-management.md`
  - `/Users/codelei/Documents/ai-project/remove-watermark/doc/engineering/change-log-standard.md`
- 实施摘要：
  - 新增 `test:shared-smoke:local-stack` 一键脚本，串联：
    - `test:shared-smoke:reset-user`（可关闭）
    - 启动 `api-gateway`（Prisma）
    - 启动 `worker-orchestrator`（Redis/BullMQ）
    - `wait-gateway` 预检查
    - 执行 `test:shared-smoke`
    - 自动输出 Markdown 报告到 `.runtime/reports`
  - 默认本地口径与本地地址保护，避免误用于非本地环境。
- 测试证据：
  - `pnpm --filter @apps/api-gateway typecheck`：通过
  - `pnpm --filter @apps/worker-orchestrator typecheck`：通过
  - `DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:5432/remove_watermark REDIS_URL=redis://127.0.0.1:6379 SHARED_BASE_URL=http://127.0.0.1:3000 pnpm --filter @apps/api-gateway test:shared-smoke:local-stack`：通过
  - 报告文件：
    - `/Users/codelei/Documents/ai-project/remove-watermark/apps/api-gateway/.runtime/reports/shared-smoke-local-stack-2026-02-22T11-25-46-878Z.md`
- 联调结果：
  - INT-006 本地验收已升级为单命令流程，复现成本与人为误差显著降低。
- 遗留问题：
  - shared/staging 云端地址未接入，本脚本仍定位于本地联调验收入口。
- 风险与回滚：
  - 风险：若本机 `3000/6379` 被占用，脚本会在 `wait-gateway` 或 smoke 阶段失败。
