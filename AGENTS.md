...
### 10.3 优化项提前执行同步规则（MUST）
- 若 `mvp-optimization-backlog.md` 中标注为 “MVP 后” 的优化项被提前在 MVP 内实现，必须在同一轮提交内同步更新：
- `执行时机`（改为实际时机）
- `状态`（至少进入 `In Review`）
- `最近更新`（更新为本次绝对日期）
- 并在 `rd-progress-management.md` 对应任务回填中显式关联该 `OPT-ID`。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| v1.11 | 2026-02-22 | 增加一键本地双进程验收命令 `@apps/api-gateway test:shared-smoke:local-stack`（reset + api + worker + smoke） |
